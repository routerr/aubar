...
func fetchSubscriptionQuota(client HTTPDoer, oauthToken string) (*SubscriptionQuota, *APIError) {
...
func probeRateLimit(client HTTPDoer, key string) (*RateLimit, *APIError) {
	...
	if claudeDir == "" {
		...
		if err != nil {
			return Output{
				...
				Errors: []APIError{{
					Period:  "local_scan",
					Message: "cannot determine home directory: " + err.Error(),
				}},
			}
		}
...
func RunCLI(key string, timeout time.Duration, claudeDir string, noProbe, noQuota bool) error {
	out := Collect(context.Background(), Options{
		APIKey:    key,
		Timeout:   timeout,
		ClaudeDir: claudeDir,
		NoProbe:   noProbe,
