...
import (
	...
	"github.com/routerr/aubar/internal/envload"
)
...
func fetchCloudCodeQuota(client HTTPDoer, accessToken string) (*QuotaResponse, error) {
...
func refreshAccessToken(client HTTPDoer, creds oauthCreds, now time.Time) (oauthCreds, error) {
...
func resolveAccessToken(client HTTPDoer, overrideToken, credsPath string, now time.Time) (string, oauthCreds, error) {
	...
	if tokenExpired(creds, now) && strings.TrimSpace(creds.RefreshToken) != "" {
		refreshed, err := refreshAccessToken(client, creds, now)
		if err != nil {
			return "", creds, err
		}
		creds = refreshed
...
func Collect(_ context.Context, opts Options) Output {
	...
	credsPath := strings.TrimSpace(opts.CredsPath)
	if credsPath == "" {
		credsPath = defaultOAuthCredsPath()
	}
	token, creds, err := resolveAccessToken(client, opts.Token, credsPath, now)
	...
	quota, err := fetchCloudCodeQuota(client, token)
	if statusErr, ok := err.(httpStatusError); ok && statusErr.StatusCode == http.StatusUnauthorized && strings.TrimSpace(opts.Token) == "" && strings.TrimSpace(creds.RefreshToken) != "" {
		refreshed, refreshErr := refreshAccessToken(client, creds, now)
		if refreshErr == nil {
			creds = refreshed
...
func PrintJSON(w io.Writer, out Output) error {
